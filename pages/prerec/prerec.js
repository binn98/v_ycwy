...
Page({
  ...
  onLoad: function (options) {
    const _that = this
    const week = new Array("周日", "周一", "周二", "周三", "周四", "周五", "周六")
    const day = new Date()
    ...
    app.ajax('logistics/receiving').then(res => {
      _that.setData({
    ...
    app.ajax('logistics/company').then(res => {
      _that.setData({
  ...
  subRec() {
    const _that = this
    if (_that.data.tab == 10 && !_that.data.date.value) {
      wx.showToast({
        title: '请选择上门日期',
        icon: 'none'
      })
